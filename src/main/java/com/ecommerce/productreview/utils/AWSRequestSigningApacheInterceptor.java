...
/**
 * Disclaimer: This code is copied from
 * https://github.com/awslabs/aws-request-signing-apache-interceptor/blob/master/src/main/java/com/amazonaws/http/AWSRequestSigningApacheInterceptor.java
 *
 * An {@link HttpRequestInterceptor} that signs requests using any AWS
 * {@link Signer} and {@link AWSCredentialsProvider}.
 */
public class AWSRequestSigningApacheInterceptor implements HttpRequestInterceptor {
    /**
     * The service that we're connecting to. Technically not necessary. Could be
     * used by a future Signer, though.
     */
     ...
     */
    AWSRequestSigningApacheInterceptor(final String service, final Signer signer,
    ...
    public void process(final HttpRequest request, final HttpContext context) throws IOException {
        ...
        signableRequest.setHeaders(headerArrayToMap(request.getAllHeaders())); // Sign it
        signer.sign(signableRequest, awsCredentialsProvider.getCredentials()); // Now copy everything back
    ...
    private static boolean skipHeader(final Header header) {
        return ("content-length".equalsIgnoreCase(header.getName()) && "0".equals(header.getValue())) // Strip
                // Content-Length:
                // 0
